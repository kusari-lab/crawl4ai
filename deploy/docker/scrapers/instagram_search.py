"""Link-only Instagram profile discovery via Google Search."""
...
class SocialLinkResult(BaseModel):
    profile_url: Optional[str] = Field(None, description="Best matching Instagram profile URL for the business")
...
class InstagramSearchScraper(BaseScraper):
    """Find an Instagram profile URL for the business (no phone extraction)."""
...
Task: find the best matching official Instagram profile URL for this business.
...
Return JSON with:
- profile_url: a single Instagram profile URL (must contain instagram.com)
...
IMPORTANT:
- Prefer instagram.com/<handle>/ profiles.
...
"""
    ...
    def _build_query(self, business_data: Dict) -> str:
        name = (business_data.get("cleaned_name") or self._get_primary_name(business_data) or "").strip()
        city = (business_data.get("MAIL_CITY") or "").strip()
        parts = [p for p in (f"site:instagram.com", name, city) if p]
    ...
    async def scrape(self, business_data: Dict, crawler=None) -> Tuple[None, None, Optional[str], Optional[Dict[str, Any]]]:
        ...
        for attempt in range(self.max_attempts):
            try:
                ...
                profile_url = self._normalize_ig_url(profile_url)
                if profile_url:
                    meta = {"social": {"instagram_url": profile_url}}
    ...
    def _normalize_ig_url(self, url: str) -> str:
        ...
        bad = ("/p/", "/reel/", "/tv/", "/explore/", "accounts/login")
